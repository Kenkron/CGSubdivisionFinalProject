...
STTriangleMesh::~STTriangleMesh()
{
	...
	for(unsigned int i = 0; i<mEdges.size(); i++)delete mEdges[i];
...
unsigned int getEdgeIndex(std::vector<STEdge*>& mEdges, const STVertex* v1, const STVertex* v2){
//	return std::find(mVertices.begin(), mVertices.end(), v) - mVertices.begin() + 1;
	int qq = 0;
	for ( qq = 0; qq < mEdges.Size(); qq++)
	{
		if ((mEdges[qq].v[0].pt.x == v1.pt.x && mEdges[qq].v[0].pt.y == v1.pt.y && mEdges[qq].v[0].pt.z == v1.pt.z) && (mEdges[qq].v[1].pt.x == v2.pt.x && mEdges[qq].v[1].pt.y == v2.pt.y && mEdges[qq].v[1].pt.z == v2.pt.z)
			||
			(mEdges[qq].v[1].pt.x == v1.pt.x && mEdges[qq].v[1].pt.y == v1.pt.y && mEdges[qq].v[1].pt.z == v1.pt.z) && (mEdges[qq].v[0].pt.x == v2.pt.x && mEdges[qq].v[0].pt.y == v2.pt.y && mEdges[qq].v[0].pt.z == v2.pt.z))
		{
			return qq;
		}
	}
		...
		while (in >> token){
			if (strcmp(token, "#") == 0){
				...
				if (mSimpleMesh){
					...
					in >> p2; in.ignore(100, ' ');
					in >> p3;
					aa = getEdgeIndex(mEdges, mVertices[p1 - 1], mVertices[p2 - 1]);
					if (aa == -1)
					{
						mEdges.push_back(new STEdge(mVertices[p1 - 1], mVertices[p2 - 1]));
						mVertices[p1 - 1].edges.push_back(new STEdge(mVertices[p1 - 1], mVertices[p2 - 1]));
						mVertices[p2 - 1].edges.push_back(new STEdge(mVertices[p1 - 1], mVertices[p2 - 1]));
					}
					aa = getEdgeIndex(mEdges, mVertices[p2 - 1], mVertices[p3 - 1]);
					if (aa == -1)
					{
						mEdges.push_back(new STEdge(mVertices[p2 - 1], mVertices[p3 - 1]));
						mVertices[p2 - 1].edges.push_back(new STEdge(mVertices[p2 - 1], mVertices[p3 - 1]));
						mVertices[p3 - 1].edges.push_back(new STEdge(mVertices[p2 - 1], mVertices[p3 - 1]));
					}
					aa = getEdgeIndex(mEdges, mVertices[p3 - 1], mVertices[p1 - 1]);
					if (aa = -1)
					{
						mEdges.push_back(new STEdge(mVertices[p3 - 1], mVertices[p1 - 1]));
						mVertices[p3 - 1].edges.push_back(new STEdge(mVertices[p3 - 1], mVertices[p1 - 1]));
						mVertices[p1 - 1].edges.push_back(new STEdge(mVertices[p3 - 1], mVertices[p1 - 1]));
					}
					mFaces.push_back(new STFace(mVertices[p1 - 1], mVertices[p2 - 1], mVertices[p3 - 1]));
					mVertices[p1 - 1].faces.push_back(new STFace(mVertices[p1 - 1], mVertices[p2 - 1], mVertices[p3 - 1]));
					mVertices[p2 - 1].faces.push_back(new STFace(mVertices[p1 - 1], mVertices[p2 - 1], mVertices[p3 - 1]));
					mVertices[p3 - 1].faces.push_back(new STFace(mVertices[p1 - 1], mVertices[p2 - 1], mVertices[p3 - 1]));
				}
				else{
					...
					mVertices[p1 - 1].faces.push_back(new STFace(mVertices[p1 - 1], mVertices[p2 - 1], mVertices[p3 - 1], mNormals[n1 - 1], mNormals[n2 - 1], mNormals[n3 - 1]));
					mVertices[p2 - 1].faces.push_back(new STFace(mVertices[p1 - 1], mVertices[p2 - 1], mVertices[p3 - 1], mNormals[n1 - 1], mNormals[n2 - 1], mNormals[n3 - 1]));
					mVertices[p3 - 1].faces.push_back(new STFace(mVertices[p1 - 1], mVertices[p2 - 1], mVertices[p3 - 1], mNormals[n1 - 1], mNormals[n2 - 1], mNormals[n3 - 1]));
				}
			}
		}
		return true;
