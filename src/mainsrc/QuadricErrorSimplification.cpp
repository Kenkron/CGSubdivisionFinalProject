...
returns -1 if this vertex is not found
*/
int findLastVertex(STTriangleMesh* mesh, STVertex* a){
	int i;
	for (i = mesh->mVertices.size()-1; i>=0; i--){
		STVertex* b = mesh->mVertices[i];
		if (a->pt.x==b->pt.x && a->pt.y==b->pt.y && a->pt.z==b->pt.z){
			return i;
		}
...
/**Finds the cost of contracting two vertices together based on the
   ...
   two vectors -Nisarg*/
double QuadricErrorSimplification::computeCost(STTriangleMesh* m, STVertex* a, STVertex* b, STVertex* w, STMatrix4* Q1, STMatrix4* Q2){
  float sum_Q[3][4];
  float temp_mat[3][4];
  ...
  double qem = w->pt.x  * (sum_Q[0][0] * w->pt.x + sum_Q[0][1] * w->pt.y + sum_Q[0][2] * w->pt.z + sum_Q[0][3]) +  
